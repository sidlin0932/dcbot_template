...
module.exports = {
    ...
    async execute(client, interaction) {
        ...
        if (bmi > 0) {
            // 如何判斷 大或小
            await interaction.reply(`利奧拉贏了！贏${bmi}平方公分！`); // 如果符合，應該回覆甚麼訊息
        } else if (bmi == 0) {
            // 應該要如何判斷剩下的
            await interaction.reply(`平手，面積都是${bmi}平方公分。`); // 如果符合，應該回覆甚麼訊息
        } else {
            // 最後一個應該是甚麼情況
            await interaction.reply(`史考特贏了:(，輸${-bmi}平方公分。`); // 如果符合，應該回覆甚麼訊息
        }
    },
};
